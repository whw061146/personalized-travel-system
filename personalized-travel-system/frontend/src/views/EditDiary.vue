<template>
  <div class="edit-diary-container">
    <el-card class="diary-card">
      ...
      <el-form v-else :model="diaryForm" :rules="rules" ref="diaryFormRef" label-width="80px">
  ...
  try {
    ...
    setTimeout(() => {
      ...
      const mockData = {
        ...
        content: '西湖真是太美了，湖水清澈，周围的景色宜人。特别是断桥残雪的景色，让人流连忘返。品尝了当地的龙井茶和西湖醋鱼，味道非常鲜美。',
        tags: ['风景', '文化', '美食'],
        images: [
          { url: 'https://example.com/image1.jpg', name: '西湖全景.jpg' },
          { url: 'https://example.com/image2.jpg', name: '断桥残雪.jpg' }
        ],
        rating: 5,
        isPublic: true
      }
      ...
      // 更新表单数据
      Object.assign(diaryForm, mockData)
      
      // 更新文件列表
      fileList.value = mockData.images.map(img => ({
        name: img.name,
        url: img.url
      }))
    ...
    if (data.success) {
      ...
      Object.assign(diaryForm, {
        ...data.diary,
        date: new Date(data.diary.date)
      })
      
      // 更新文件列表
      fileList.value = data.diary.images.map(img => ({
        name: img.name,
        url: img.url
      }))
    } else {
      ElMessage.error(data.message || '获取日记数据失败')
      router.push('/diary')
    }
    */
  } catch (error) {
...
if (!userStore.isLoggedIn) {
  ...
  router.push(`/login?redirect=/diary/edit/${diaryId}`)
}
